...
document.querySelector("button").addEventListener("click", (e) => {
  const 제목 = document.querySelector(".title").value;
  const 할일 = document.querySelector(".todos").value;
  const 링크 = document.querySelector(".URL").value;
  const 컬러 = 색추출기(colors);
  // local storage 저장
  const todo객체 = {};
  todo객체.제목 = 제목;
  todo객체.할일 = 할일;
  ...
  // 새로운 태그 생성
  const newTag = createTag(제목, 할일, 링크, 컬러, todo객체.id);
...
function createTag(제목, 할일, 링크, 컬러, key) {
  // 새로운 p 태그요소를 생성
  const newTag = document.createElement("div");
  newTag.style.backgroundColor = 컬러;

  const 제목임 = document.createElement("h4");
  제목임.textContent = 제목;
  newTag.appendChild(제목임);

  const 할일임 = document.createElement("p");
  할일임.textContent = 할일;
  newTag.appendChild(할일임);

  const 링크임 = document.createElement("a");
  링크임.href = 링크;
  링크임.target = "_blank";
  링크임.innerHTML = 링크;
  newTag.appendChild(링크임);
...
==> drop 이벤트 함수에 추가!!!
---------------------------------------------------------------------------
1-1. HTML box class 안에 category="todo" category="doing" category="done"
     를 추가.
     "drop" 이벤트에서  e.currentTarget는 드롭을 당하는 박스 자체(p태그 포함)
     .appendChild(dragTarget)의 dragTarget은 p.

1-3 key 값은 todo객체.id = Date.now();이고 이 key값을 다른곳에서 읽기 위해서 
    newTag.setAttribute("key", todo객체.id);를 추가한다. 

1-4 그 후 const todo = localStorage.getItem()의 ()안에 
    dragTarget.getAttribute("key")를 추가!

1-5 const todo = localStorage.getItem(); 앞에 JSON.parse를 추가하여 
    const todo = JSON.parse(localStorage.getItem(dragTarget.getAttribute
    ("key"))); 이 된다.

1-6 todo.category = e.currentTarget.getAttribute("category"); 까지 추가하면 
    이제 드롭을 할 시, console.log(todo)에서  카테고리가 맞게 바뀌는걸 볼 수 
    있다. 

    todo.category = e.currentTarget은 <div class="box! 

1-7 localStorage.setItem(todo.id, JSON.stringify(todo)); 를 추가하면 
    Application에서 카테고리가 바뀌는걸 확인할 수 있다.
---------------------------------------------------------------------------
...
2-1 localstorage.removeItem(key) 이용.
    key 값을 얻기 위해 현재 클릭 이벤트가 발생되고 있는 부모의 속성에서 
    key값을 읽는다
    const key = e.currentTarget.parentElement.getAttribute("key");

2-2 localStorage.removeItem(key);까지 추가 후 새로 만들어지는 리스트들부터 
    X버튼 누를 시 Application에서에서도 데이터가 삭제된다.
---------------------------------------------------------------------------
...
1. const colors = [...]: 색상 값을 담고 있는 배열입니다. 각 색상은 16진수 형태로 
   표현되어 있습니다.

2. let dragTarget = null;: 드래그 대상을 저장하는 변수입니다. 초기값은 null로 
   설정되어 있습니다.

3. display(): 화면에 저장된 데이터를 표시하는 함수입니다. localStorage에 저장된 
   데이터를 읽어와서 해당 데이터에 맞는 태그를 생성하여 화면에 추가합니다.

4. 색추출기(colors): colors 배열에서 랜덤한 색상을 선택하여 반환하는 함수입니다.

5. document.querySelector("button").addEventListener("click", (e) => 
   { ... });: 버튼의 클릭 이벤트를 처리하는 부분입니다. 사용자가 입력한 텍스트를 
   가져와서 새로운 태그를 생성하여 화면에 추가하고, 입력값을 초기화합니다. 또한, 
   localStorage에도 입력값을 저장합니다.

6. const boxes = document.querySelectorAll(".box");: .box 클래스를 가지는 
   요소들을 모두 선택합니다. 이 요소들은 드롭 영역을 나타냅니다.

7. boxes.forEach((box, i) => { ... });: 선택한 .box 요소들에 대해 각각 드래그 
   관련 이벤트를 처리하는 부분입니다. 드래그 영역에서 드래그를 허용하도록 설정하고,
   드롭이 발생했을 때 해당 요소에 드래그 대상을 추가하고 localStorage에 해당 
   데이터의 카테고리를 업데이트합니다.

8. createTag(text, key): 새로운 태그를 생성하는 함수입니다. 함수에 전달된 
   텍스트와 키 값을 사용하여 새로운 <p> 태그를 생성하고, 색상을 랜덤하게 
   설정합니다. 또한, 드래그 이벤트와 삭제 버튼 클릭 이벤트를 설정하여 기능을 
   추가합니다. 생성된 태그에는 고유한 키 값도 설정됩니다.

9. display(): 초기화면 로딩 시에 호출되어 localStorage에 저장된 데이터를 
   읽어와서 화면에 표시합니다. 각 데이터에 해당하는 새로운 태그를 생성하여 해당 
   카테고리 영역에 추가합니다.
...
---------------------------------------------------------------------------
  ...
  업데이트된 코드에서 주요한 변경 사항은 다음과 같습니다:

  로컬 스토리지 통합: localStorage를 사용하여 할 일 목록 항목을 저장하고 
  검색하는 로컬 스토리지 통합이 추가되었습니다.

  createTag() 함수: createTag() 함수가 추가되어 새로운 할 일 항목을 생성하고 
  이를 화면에 표시하는 기능을 수행합니다.

  이벤트 리스너: 드래그 앤 드롭 기능을 위한 이벤트 리스너가 수정되어, 
  항목을 드롭할 때 해당 항목의 카테고리 정보를 업데이트하고 
  로컬 스토리지에 저장합니다.

  display() 함수: display() 함수가 추가되어 페이지 로딩 시 로컬 스토리지에서 
  데이터를 읽어와 화면에 표시합니다.

  이러한 변경 사항은 로컬 스토리지를 활용하여 데이터를 유지하고, 
  새로운 항목을 생성하고 삭제할 수 있는 기능을 추가하는 데 사용되었습니다.
---------------------------------------------------------------------------
*/
